...
import * as gt from './chara_list_n_nottyojin.js';
...
function processGacha(seed, rounds, arr, valueFn, nameList, recordList, darkList, labelOffsets, gatya, checkDark = false) {
  ...
  for (let i = 0; i <= rounds; i++) {
    ...
    let daburu = "F", tripuru = "F", one = "F", targetseedNm = 0;
    ...
    if (arr === gt.LCG) console.log(i, daburu, tripuru, one)
    ...
    if (name) n_seed_link = `<a href="${baseUrl}?seed=${seed}&type=${gatya}" style="color:${getTextColor(name)}">${name}</a>`;
    if (checkDark) {
      darkList.push(r1 >= 9900 ? "T" : "F");
      if (r1 >= 9900 && gatya !== "1") k_seed_link += "<br>(CE→闇猫目)";
    }
    nameList.push(n_seed_link + k_seed_link);
    recordList.push(kaburi ? (tripuru === "T" ? "3" : daburu === "T" ? "2" : "1") : "0");
    mae_seed = name;
  }
}
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (gatya === "0") {
    ...
    processGacha(seed, 100, gt.LCG, getValueth, namesga, [], [], { single: "B", double: "A", triple: "B", add: 2 }, gatya);
    processGacha(new Xorshift32(seed).random(), 100, gt.LCG, getValueth, namesgb, [], [], { single: "A", double: "B", triple: "A", add: 3 }, gatya);
  } else {
    processGacha(seed, 100, gt.M, getValuesc, namesca, [], [], { single: "B", double: "A", triple: "B", add: 2 }, gatya);
    processGacha(new Xorshift32(seed).random(), 100, gt.M, getValuesc, namescb, [], [], { single: "A", double: "B", triple: "A", add: 3 }, gatya);
    processGacha(seed, 100, gt.CE, getValueth, namesga, [], [], { single: "B", double: "A", triple: "B", add: 2 }, gatya);
